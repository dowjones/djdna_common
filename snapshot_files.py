...
ARTICLES_STAT_FIELDS = ['an','company_codes','company_codes_about','industry_codes','ingestion_datetime',
                        'language_code','modification_date','modification_datetime',
                        'publication_date','publication_datetime','publisher_name',
...
def read_file(filepath, only_stats = True, merge_body = True) -> pd.DataFrame:
    ...
    only_stats : bool, optional
        Specifies if only file metadata is loaded (True), or if the full article content is loaded (False). On average, only_stats loads about 1/10 and is recommended for quick metadata-based analysis. (Default is True)
    merge_body : bool, optional
        Specifies if the body field should be merged with the snippet and this last column being dropped. (default is True)
    Returns
    -------
    pandas.DataFrame
    ...
    if (only_stats == False) & (merge_body == True):
        r_df['body'] = r_df['snippet'] + "\n\n" + r_df['body']
        r_df.drop('snippet', axis=1, inplace=True)

    r_df.drop(ARTICLE_DELETE_FIELDS, axis=1, inplace=True)
